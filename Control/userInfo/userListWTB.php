...
// lay ket qua
$query = "SELECT * FROM buyer where (User = '{$user}')  and ItemName like '%".$item."%' ORDER BY `create_date` DESC";
...
while($row = mysqli_fetch_array($result)){
	...
	<div class="row">
	   ...
	   <label class="col-2">';
       if($bool == "true"){
            $itemShow .= '<form class="deleteItem" method="get" onclick="deleteList('.$row["id"].', \'buyer\');return false;">
                <i class="fas fa-trash-alt"></i>
                <input id="deletedUser" hidden type="text" value="'.$row["id"].'">
            </form>';
       }
    $itemShow .= '<input type="button" class="fas itemBtn itemBtnBuy" name="" value="&#xf07a;  BUY">
	   </label>
	</div>
 </div>  ';
	$white++;
